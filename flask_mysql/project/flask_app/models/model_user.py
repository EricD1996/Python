...
class User:
    ...
    def validator(data:dict) -> bool:
        ...
        if len(data['first_name']) < 2:
            flash('field is required, must be at least 2 characters', 'err_users_first_name')
            is_valid = False


        if len(data['last_name']) < 2:
            flash('field is required, must be at least 2 characters', 'err_users_last_name')
            is_valid = False
        ...
        if len(data['pw']) < 8:
            flash('field is required, password must be at least 8 characters', 'err_users_pw')
            is_valid = False
